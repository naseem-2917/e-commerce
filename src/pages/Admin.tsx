...
import { Plus, Trash2, Edit2, Package, ShoppingBag, Tag, X, Check, ToggleLeft, ToggleRight, Shield, Key, Copy, CheckCircle, User } from 'lucide-react';
import { MOCK_PRODUCTS, MOCK_ORDERS, MOCK_COUPONS, CATEGORIES } from '../data/mock';
import { Product, Coupon } from '../types';
import { motion, AnimatePresence } from 'framer-motion';

// Admin secret code for promotion
const ADMIN_SECRET_CODE = 'nasadmin';
...
export default function Admin() {
    ...
    const copyToClipboard = () => {
        navigator.clipboard.writeText(ADMIN_SECRET_CODE);
        setCopied(true);
    ...
    const handleAdminPromotion = async (e: React.FormEvent) => {
        ...
        if (secretCode === ADMIN_SECRET_CODE) {
            ...
            setAdminError('Invalid secret code. Try again.');
        }
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
                ...
                >
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 border border-slate-200 dark:border-slate-800">
                        ...
                        <form onSubmit={handleAdminPromotion} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Admin Secret Code</label>
                        ...
                        <div className="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
                            <h3 className="font-bold text-amber-800 dark:text-amber-400 mb-3 text-sm">🔑 Test Secret Code (Click to copy)</h3>
                            <div
                                onClick={copyToClipboard}
                                className="flex items-center justify-between bg-white dark:bg-slate-800 px-4 py-3 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
                            >
                                <div>
                                    <span className="text-xs text-slate-500 dark:text-slate-400">Secret Code:</span>
                                    <p className="font-mono text-lg font-bold text-slate-900 dark:text-white">{ADMIN_SECRET_CODE}</p>
                                </div>
                                {copied ? <CheckCircle size={20} className="text-green-500" /> : <Copy size={20} className="text-slate-400" />}
                            </div>
                            <p className="text-xs text-amber-700 dark:text-amber-300 mt-3">
                                ✅ Enter this code and your account will become admin!
                            </p>
                        </div>
                    </div>
                </motion.div>
